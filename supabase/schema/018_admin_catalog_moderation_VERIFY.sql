-- KÖL / 018 admin catalog moderation verification
...
BEGIN
  ...
  if v_public_definer is null then
    raise exception '018 verify: public moderation RPC missing';
  end if;
  if v_public_definer is distinct from false or v_public_fixed_path is distinct from true then
    raise exception '018 verify: public moderation RPC must remain SECURITY INVOKER with fixed search_path';
  end if;
  ...
  if v_private_definer is distinct from true or v_private_fixed_path is distinct from true then
    raise exception '018 verify: private moderation function must be SECURITY DEFINER with fixed search_path';
  end if;

  if pg_catalog.has_function_privilege('anon', 'public.admin_catalog_moderation_atomic(uuid,text,text,text,text)', 'EXECUTE') then
    raise exception '018 verify: anon must not execute public moderation RPC';
  end if;
  if not pg_catalog.has_function_privilege('authenticated', 'public.admin_catalog_moderation_atomic(uuid,text,text,text,text)', 'EXECUTE') then
    raise exception '018 verify: authenticated wrapper EXECUTE grant missing';
  end if;
  ...
  if pg_catalog.has_table_privilege('authenticated', 'public.menu_items', 'INSERT')
    ...
    raise exception '018 verify: direct authenticated catalog INSERT/UPDATE/DELETE must remain revoked';
  end if;
END
$$;

select '018 admin catalog moderation verification: PASS' as result;
