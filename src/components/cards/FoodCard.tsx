...
export function FoodCard({ food, partnerName, prepTime = "25-35 мин", className }: FoodCardProps) {
  return (
    <Card className={cn("group overflow-hidden transition duration-200 hover:-translate-y-1 hover:shadow-soft", className)}>
      ...
      <CardContent className="space-y-4 p-5">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2"><Badge variant="muted">{food.category}</Badge><Badge variant="info">{prepTime}</Badge></div>
