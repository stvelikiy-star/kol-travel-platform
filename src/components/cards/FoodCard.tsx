...
export function FoodCard({ food, partnerName, partnerSlug, prepTime = "25-35 мин", className }: FoodCardProps) {
  ...
  return (
    <Card className={cn("group overflow-hidden transition duration-200 hover:-translate-y-1 hover:shadow-soft", className)}>
      ...
      <CardFooter>
        <a className="inline-flex min-h-11 w-full items-center justify-center rounded-md border border-primary bg-primary px-4 py-2 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(15,143,140,0.22)] transition hover:shadow-[0_10px_24px_rgba(15,143,140,0.28)] aria-disabled:pointer-events-none aria-disabled:opacity-50" aria-disabled={!canOpen} href={partnerSlug ? `/food/${partnerSlug}` : "#"}>Открыть меню</a>
      </CardFooter>
    </Card>
  );
}
