    ...
    <main className="min-h-screen bg-gradient-to-br from-background via-lake-light/45 to-sand-light/60">
      <div className="border-b border-border/80 bg-surface/90 shadow-sm backdrop-blur-xl">
        <Container className="flex min-h-20 flex-wrap items-center justify-between gap-4 py-4">
          <div><p className="text-xs font-semibold uppercase tracking-wide text-primary">KÖL Client</p><h1 className="text-2xl font-semibold leading-tight text-foreground">Кабинет клиента</h1></div>
        ...
        <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
          <Card className="overflow-hidden shadow-soft">
            <div className="bg-gradient-to-br from-lake-dark via-primary to-sand p-5 text-white">
              ...
              <div className="mt-4"><p className="text-lg font-semibold">KÖL Client</p><p className="text-sm text-white/80">Единый аккаунт путешественника</p></div>
            </div>
            <CardContent className="space-y-3 p-4">
              <div className="grid grid-cols-2 gap-2"><Metric label="Баллы" value="1 240" /><Metric label="Предложения" value="4" /></div>
              <div className="rounded-md border border-warning/40 bg-warning/10 p-3 text-sm font-medium text-foreground">Режим просмотра: личные данные и операции аккаунта не записываются без реальной авторизации пользователя.</div>
            </CardContent>
          </Card>

          <Card className="hidden lg:block shadow-card">
            <CardHeader className="pb-3"><div className="flex items-center justify-between gap-3"><CardTitle className="text-base">Навигация</CardTitle><Badge variant="info">Клиент</Badge></div></CardHeader>
...
function isActive(pathname: string, href: string) { if (href === "/client") return pathname === href; return pathname.startsWith(href); }
