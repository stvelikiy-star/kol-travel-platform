        ...
        <nav className="hidden items-center gap-3 xl:flex">
          {publicLinks.map((link) => (
            <Link className="rounded-md px-2 py-2 text-sm font-semibold text-muted transition hover:bg-lake-light hover:text-primary" href={link.href} key={link.href}>
              {link.href === "/cart" && cart.hydrated && cart.itemCount > 0 ? `${link.label} (${cart.itemCount})` : link.label}
            </Link>
          ))}
        </nav>

        <div className="hidden items-center gap-2 lg:flex">
          <Link className="inline-flex min-h-11 items-center justify-center rounded-md border border-transparent bg-transparent px-4 py-2 text-sm font-semibold text-foreground transition hover:bg-lake-light hover:text-primary" href="/client">Войти</Link>
          <Link className="inline-flex min-h-11 items-center justify-center rounded-md border border-primary bg-primary px-4 py-2 text-sm font-semibold text-white shadow-[0_8px_20px_rgba(15,143,140,0.22)] transition hover:shadow-[0_10px_24px_rgba(15,143,140,0.28)]" href="/partner">Кабинет партнёра</Link>
        </div>
