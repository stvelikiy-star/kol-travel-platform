...
type PartnerLayoutProps = { children: ReactNode; className?: string };
    ...
    <main className="min-h-screen bg-gradient-to-br from-background via-lake-light/45 to-sand-light/60">
      <div className="border-b border-border/80 bg-surface/90 shadow-sm backdrop-blur-xl">
        <Container className="flex min-h-20 flex-wrap items-center justify-between gap-4 py-4">
          <div><p className="text-xs font-semibold uppercase tracking-wide text-primary">KÖL Partner</p><h1 className="text-2xl font-semibold leading-tight text-foreground">Кабинет партнёра</h1></div>
        ...
        <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
          <Card className="overflow-hidden shadow-soft">
            <div className="bg-gradient-to-br from-lake-dark via-primary to-sand p-5 text-white">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-white/20 text-lg font-semibold">KP</div>
              <div className="mt-4"><p className="text-lg font-semibold">KÖL Partner</p><p className="text-sm text-white/80">Рабочее пространство бизнеса</p></div>
            </div>
            <CardContent className="space-y-3 p-4">
              <div className="grid grid-cols-2 gap-2"><Metric label="Статус" value="online" /><Metric label="Рейтинг" value="4.8" /></div>
              <div className="rounded-md border border-warning/40 bg-warning/10 p-3 text-sm font-medium">Режим просмотра: кабинет показывает структуру бизнеса. Неподдержанные изменения отключены до подключения авторизации, RLS и серверных операций.</div>
            </CardContent>
          </Card>
          ...
          <Card className="hidden lg:block shadow-card">
            <CardHeader className="pb-3"><div className="flex items-center justify-between gap-3"><CardTitle className="text-base">Навигация</CardTitle><Badge variant="success">Партнёр</Badge></div></CardHeader>
...
function isActive(pathname: string, href: string) { if (href === "/partner") return pathname === href; return pathname.startsWith(href); }
