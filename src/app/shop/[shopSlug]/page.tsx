        ...
        <section className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-6">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <Card><CardContent className="p-4"><p className="text-sm text-muted">Товары</p><p className="font-semibold">{products.length}</p></CardContent></Card>
              <Card><CardContent className="p-4"><p className="text-sm text-muted">Категории</p><p className="font-semibold">{categories.length}</p></CardContent></Card>
          ...
          <div className="aspect-[4/3] rounded-lg bg-gradient-to-br from-[#d7b56d] via-accent to-primary p-5 text-white"><div className="flex h-full flex-col justify-between"><Badge className="border-white/40 bg-white text-primary">KÖL Shop</Badge><p className="text-2xl font-semibold">Нужные товары рядом</p></div></div>
        </section>

        {isUnavailable ? <Card className="border-warning"><CardContent className="p-5 text-sm leading-6 text-muted">Магазин временно не принимает новые заказы. Уже принятые заказы продолжают обрабатываться по своему статусу.</CardContent></Card> : null}
        {hasUnavailableProducts ? <Card className="border-warning"><CardContent className="p-5 text-sm leading-6 text-muted">Часть товаров временно недоступна. Добавление таких позиций в заказ отключено.</CardContent></Card> : null}

        <section className="space-y-5"><SectionTitle title="Категории товаров" description="Быстрый выбор нужного раздела магазина." /><div className="flex flex-wrap gap-2">{categories.map((category) => <Badge key={category} variant="muted">{category}</Badge>)}</div></section>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {products.map((product) => (
                  <div className="space-y-4" key={product.id}>
                    <ProductCard partnerName={partner.title} partnerSlug={partner.slug} product={product} stockLabel={product.status === "active" ? "В наличии" : "Недоступно"} />
                    <AddToCartPanel businessId={product.businessId} currency={product.currency} itemId={product.id} itemType="product" partnerName={partner.title} price={product.price} status={isUnavailable ? "stopped" : product.status} title={product.title} />
                  </div>
                ))}
              </div>
              <CartSummaryPreview />
            </div>
          ) : <EmptyState actionLabel="Вернуться в магазин" description="Каталог этого магазина пока не опубликован." href="/shop" title="Товары обновляются" />}
        </section>
      </Container>
      <PublicFooter />
    </main>
