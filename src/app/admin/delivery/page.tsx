...
type AdminDeliverySearchParams = {
...
async function assignCourierFromForm(formData: FormData) {
  ...
  const params = new URLSearchParams({
    assignment: result.ok ? "success" : "error",
...
export default async function AdminDeliveryPage({
  ...
  searchParams?: Promise<AdminDeliverySearchParams>;
}) {
    ...
    <AdminLayout status={problems > 0 || unavailable ? "attention" : "stable"}>
      <Card className="overflow-hidden">
        <div className="bg-gradient-to-br from-slate-900 via-primary to-accent p-6 text-white">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-white/85">
            Администратор видит реальные заказы и состояние доставки, назначает доступного курьера через защищённую серверную операцию и контролирует дальнейший маршрут.
          </p>
        </div>
      </Card>

      {assignmentResult ? (
        <Card className={assignmentResult === "success" ? "border-success/40 bg-success/10" : "border-danger/40 bg-danger/10"}>
          <CardContent className="p-4 text-sm font-medium leading-6 text-foreground" role="status">
            {assignmentResult === "success"
              ? `Курьер назначен сервером${assignmentDeliveryId ? ` для доставки ${assignmentDeliveryId}` : ""}.`
              : `Назначение курьера отклонено безопасно${assignmentCode ? `: ${assignmentCode}` : "."}`}
          </CardContent>
        </Card>
      ) : null}
      ...
      <Card className={unavailable ? "border-danger/40 bg-danger/10" : "border-primary/20 bg-surface"}>
        <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm">
          <Badge variant={readResult.source === "supabase" ? "success" : "warning"}>
            {readResult.source === "supabase" ? "Supabase · рабочие данные" : "Операции отключены"}
          </Badge>
          <p className="max-w-3xl leading-6 text-muted">
            {unavailable
              ? "Рабочие данные доставки сейчас недоступны. Назначения не выполняются без подтверждённого Supabase-источника."
              : readResult.source === "supabase"
                ? `Получены реальные доставки и ${couriers.length} доступных курьеров.`
                : "Вне Supabase-режима экран доступен только для просмотра; реальные назначения не имитируются."}
          </p>
        </CardContent>
      </Card>
      ...
      <section className="grid gap-4">
        {deliveryOrders.map((order) => (
          <DeliveryCard couriers={couriers} key={order.id} order={order} supabaseMode={readResult.source === "supabase"} />
        ))}
...
function DeliveryCard({
  ...
  order: AdminOperationalDeliveryOrder;
  supabaseMode: boolean;
}) {
  const currentStatus = order.deliveryStatus ?? order.status;
  const canAssign = Boolean(order.deliveryId) && assignableDeliveryStatuses.has(currentStatus) && couriers.length > 0 && supabaseMode;
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Badge variant={statusVariant[currentStatus] ?? "muted"}>{currentStatus}</Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Info label="ID доставки" value={order.deliveryId ?? "Доставка ещё не создана"} />
          ...
          <Info label="Назначенный курьер" value={order.assignedCourierId ?? "Не назначен"} />
          <Info label="Сумма" value={`${order.total} KGS`} />
          <Info label="Обновлено" value={new Date(order.updatedAt).toLocaleString("ru-RU")} />
        </div>

        {canAssign ? (
          <form action={assignCourierFromForm} className="grid gap-3 rounded-md border border-primary/20 bg-lake-light p-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
            <input name="deliveryId" type="hidden" value={order.deliveryId} />
            <label className="grid gap-2 text-sm font-semibold text-foreground">
              Курьер
              <select
                aria-label={`Курьер для доставки ${order.deliveryId}`}
                className="min-h-11 rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground"
                name="courierId"
                required
              >
                <option value="">Выберите доступного курьера</option>
                {couriers.map((courier) => (
                  <option key={courier.userId} value={courier.userId}>
                    {courier.fullName ?? courier.email ?? courier.userId} · {courier.availabilityStatus}{courier.vehicleNumber ? ` · ${courier.vehicleNumber}` : ""}
                  </option>
                ))}
              </select>
            </label>
        ...
        {supabaseMode && order.deliveryId && assignableDeliveryStatuses.has(currentStatus) && couriers.length === 0 ? (
          <p className="rounded-md border border-warning/30 bg-warning/10 p-3 text-sm font-medium text-foreground">
            Сейчас нет курьеров со статусом online/available. Назначение недоступно до появления реального доступного курьера.
          </p>
        ) : null}
      </CardContent>
    </Card>
