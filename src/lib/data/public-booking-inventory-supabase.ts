...
export async function getPublicStayInventoryFromSupabase(
  ...
  if (!uuidPattern.test(stayId)) {
    return {
      ...
      message: "Stay identifier is invalid."
    };
  }

  if (!getSupabaseReadConfig()) {
    return {
      ok: false,
      source: "supabase",
      rooms: [],
  ...
  try {
    ...
    const rows = await callPublicRpc<StayInventoryRpcRow>("get_public_stay_inventory", {
      p_stay_id: stayId,
      p_from: window.from,
      p_to: window.to
    });
    ...
    for (const row of rows) {
      ...
      if (
        ...
        return {
          ...
          message: "Public Stay inventory response failed validation."
        };
      }
      ...
      if (row.availability_date !== null && row.availability_status !== null) {
        ...
        if (
          ...
          return {
            ...
            availability: [],
            code: "invalid_response",
            message: "Public Stay availability response failed validation."
          };
        }
    ...
    return {
      ...
      availability,
      message: "Public Stay booking inventory read through constrained RPC."
    };
  } catch {
    return {
      ok: false,
      ...
      availability: [],
      code: "server_error",
      message: "Public Stay inventory read failed safely."
    };
  }
}
...
export async function getPublicTourSchedulesFromSupabase(
  ...
  if (!uuidPattern.test(tourId)) {
    return {
      ...
      schedules: [],
      code: "invalid_identifier",
      message: "Tour identifier is invalid."
    };
  }
  ...
  try {
    const window = getReadWindow();
    const rows = await callPublicRpc<TourScheduleRpcRow>("get_public_tour_schedules", {
      p_tour_id: tourId,
      p_from: window.from,
      p_to: window.to
    });
    ...
    for (const row of rows) {
      ...
      if (
        !uuidPattern.test(row.schedule_id) ||
        row.tour_id !== tourId ||
        !row.schedule_date ||
        capacity === null ||
        !Number.isInteger(capacity) ||
        ...
        return {
          ...
          schedules: [],
          code: "invalid_response",
          message: "Public Tour schedules response failed validation."
        };
      }
    ...
    return {
      ...
      source: "supabase",
      schedules,
      message: "Public Tour schedules read through constrained RPC."
    };
  } catch {
    return {
      ok: false,
      ...
      schedules: [],
      code: "server_error",
      message: "Public Tour schedules read failed safely."
    };
  }
}
