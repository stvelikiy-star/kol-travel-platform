...
APP_PORT="${KOL_LOCAL_ADMIN_GOVERNANCE_PORT:-3106}"
APP_BASE_URL="http://127.0.0.1:${APP_PORT}"
APP_LOG="${RUNNER_TEMP:-/tmp}/kol-admin-catalog-governance-runtime.log"
APP_PID=""
...
case "$DB_URL" in
  *"@127.0.0.1:"*|*"@localhost:"*) ;;
  *) echo "Refusing Admin catalog governance smoke against non-local database." >&2; exit 1 ;;
esac

touch supabase/seed.sql
supabase start -x studio,imgproxy,realtime,edge-runtime,logflare,vector
supabase status -o env > /tmp/kol-admin-governance-supabase.env
set -a
# shellcheck disable=SC1091
source /tmp/kol-admin-governance-supabase.env
set +a
...
if [[ "$ready" -ne 1 ]]; then
  echo "KÖL Admin catalog governance application did not become ready." >&2
  cat "$APP_LOG" >&2 || true
  exit 1
fi

node scripts/qa-local-admin-catalog-governance-runtime.mjs "$APP_BASE_URL"
echo "KÖL local Admin catalog governance staging smoke: PASS"
