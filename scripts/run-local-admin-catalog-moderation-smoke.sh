...
APP_PORT="${KOL_LOCAL_ADMIN_CATALOG_PORT:-3104}"
APP_BASE_URL="http://127.0.0.1:${APP_PORT}"
APP_LOG="${RUNNER_TEMP:-/tmp}/kol-admin-catalog-moderation-runtime.log"
APP_PID=""
...
case "$DB_URL" in
  *"@127.0.0.1:"*|*"@localhost:"*) ;;
  *) echo "Refusing Admin catalog moderation smoke against non-local database." >&2; exit 1 ;;
esac

touch supabase/seed.sql
supabase start -x studio,imgproxy,realtime,edge-runtime,logflare,vector
supabase status -o env > /tmp/kol-admin-catalog-supabase.env
set -a
# shellcheck disable=SC1091
source /tmp/kol-admin-catalog-supabase.env
set +a
...
psql "$DB_URL" -X -v ON_ERROR_STOP=1 -f supabase/schema/016_admin_catalog_moderation_VERIFY.sql >/dev/null
...
if [[ "$ready" -ne 1 ]]; then
  echo "KÖL Admin catalog moderation application did not become ready." >&2
  cat "$APP_LOG" >&2 || true
  exit 1
fi

node scripts/qa-local-admin-catalog-moderation-runtime.mjs "$APP_BASE_URL"
echo "KÖL local Admin catalog moderation staging smoke: PASS"
