...
APP_PORT="${KOL_LOCAL_ADMIN_SUPPORT_PORT:-3107}"
APP_BASE_URL="http://127.0.0.1:${APP_PORT}"
APP_LOG="${RUNNER_TEMP:-/tmp}/kol-admin-support-runtime.log"
APP_PID=""
...
case "$DB_URL" in
  *"@127.0.0.1:"*|*"@localhost:"*) ;;
  *) echo "Refusing Admin Support smoke against non-local database." >&2; exit 1 ;;
esac

touch supabase/seed.sql
supabase start -x studio,imgproxy,realtime,edge-runtime,logflare,vector
supabase status -o env > /tmp/kol-admin-support-supabase.env
set -a
# shellcheck disable=SC1091
source /tmp/kol-admin-support-supabase.env
set +a
...
psql "$DB_URL" -X -v ON_ERROR_STOP=1 -f supabase/schema/022_admin_support_lifecycle_VERIFY.sql >/dev/null
...
if [[ "$ready" -ne 1 ]]; then
  echo "KÖL Admin Support application did not become ready." >&2
  cat "$APP_LOG" >&2 || true
  exit 1
fi

node scripts/qa-local-admin-support-runtime.mjs "$APP_BASE_URL"
echo "KÖL local Admin Support staging smoke: PASS"
