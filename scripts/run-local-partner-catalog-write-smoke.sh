...
APP_PORT="${KOL_LOCAL_PARTNER_CATALOG_WRITE_PORT:-3105}"
APP_BASE_URL="http://127.0.0.1:${APP_PORT}"
APP_LOG="${RUNNER_TEMP:-/tmp}/kol-partner-catalog-write-runtime.log"
APP_PID=""
...
case "$DB_URL" in
  *"@127.0.0.1:"*|*"@localhost:"*) ;;
  *) echo "Refusing Partner catalog write smoke against non-local database." >&2; exit 1 ;;
esac

touch supabase/seed.sql
supabase start -x studio,imgproxy,realtime,edge-runtime,logflare,vector
supabase status -o env > /tmp/kol-partner-catalog-write-supabase.env
set -a
# shellcheck disable=SC1091
source /tmp/kol-partner-catalog-write-supabase.env
set +a
...
for (const migration of manifest.migrations) process.stdout.write(`${migration.id}\t${migration.apply}\n`);
NODE
)
...
psql "$DB_URL" -X -v ON_ERROR_STOP=1 -f supabase/schema/019_partner_catalog_write_runtime_VERIFY.sql >/dev/null
...
if [[ "$ready" -ne 1 ]]; then
  echo "KÖL Partner catalog write application did not become ready." >&2
  cat "$APP_LOG" >&2 || true
  exit 1
fi

node scripts/qa-local-partner-catalog-write-runtime.mjs "$APP_BASE_URL"
echo "KÖL local Partner catalog write staging smoke: PASS"
