...
APP_PORT="${KOL_LOCAL_CLIENT_SUPPORT_PORT:-3106}"
APP_BASE_URL="http://127.0.0.1:${APP_PORT}"
APP_LOG="${RUNNER_TEMP:-/tmp}/kol-client-support-runtime.log"
APP_PID=""
...
case "$DB_URL" in
  *"@127.0.0.1:"*|*"@localhost:"*) ;;
  *) echo "Refusing Client Support smoke against non-local database." >&2; exit 1 ;;
esac

touch supabase/seed.sql
supabase start -x studio,imgproxy,realtime,edge-runtime,logflare,vector
supabase status -o env > /tmp/kol-client-support-supabase.env
set -a
# shellcheck disable=SC1091
source /tmp/kol-client-support-supabase.env
set +a
...
psql "$DB_URL" -X -v ON_ERROR_STOP=1 -f supabase/schema/021_client_support_runtime_VERIFY.sql >/dev/null
...
if [[ "$ready" -ne 1 ]]; then
  echo "KÖL Client Support application did not become ready." >&2
  cat "$APP_LOG" >&2 || true
  exit 1
fi

node scripts/qa-local-client-support-runtime.mjs "$APP_BASE_URL"
echo "KÖL local Client Support staging smoke: PASS"