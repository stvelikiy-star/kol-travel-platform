...
if [[ -z "$PUBLIC_KEY" ]]; then echo "Local public Supabase key is required for runtime smoke." >&2; exit 1; fi
case "$NEXT_PUBLIC_SUPABASE_URL" in http://127.0.0.1:*|http://localhost:*) ;; *) echo "Refusing runtime fixtures against a non-local Supabase URL." >&2; exit 1;; esac
case "$LOCAL_DB_URL" in *"@127.0.0.1:"*|*"@localhost:"*) ;; *) echo "Refusing runtime fixtures against a non-local database URL." >&2; exit 1;; esac
...
APP_PID=""
cleanup_app() { if [[ -n "$APP_PID" ]]; then kill "$APP_PID" >/dev/null 2>&1 || true; wait "$APP_PID" >/dev/null 2>&1 || true; fi; }
trap cleanup_app EXIT

assert_rest_row() {
  local table="$1" title="$2" status_filter="$3" body
  body="$(curl -fsS --get -H "apikey: ${PUBLIC_KEY}" -H "accept: application/json" --data-urlencode "select=title" --data-urlencode "status=${status_filter}" "${NEXT_PUBLIC_SUPABASE_URL%/}/rest/v1/${table}")"
  if [[ "$body" != *"\"title\":\"${title}\""* ]]; then echo "Anon REST smoke failed for ${table}; expected '${title}'." >&2; echo "$body" >&2; exit 1; fi
  echo "Anon REST ${table}: PASS"
}
assert_rpc_contains() {
  local function_name="$1" request_body="$2"; shift 2; local body expected
  body="$(curl -fsS -X POST -H "apikey: ${PUBLIC_KEY}" -H "accept: application/json" -H "content-type: application/json" --data "$request_body" "${NEXT_PUBLIC_SUPABASE_URL%/}/rest/v1/rpc/${function_name}")"
  for expected in "$@"; do if [[ "$body" != *"$expected"* ]]; then echo "Anon RPC smoke failed for ${function_name}; expected '${expected}'." >&2; echo "$body" >&2; exit 1; fi; done
...
insert into public.room_availability (room_id,date,status,available_count,price_override) values ('42000000-0000-0000-0000-000000000001',current_date + 7,'available',2,5700)
...
insert into public.tour_schedules (id,tour_id,date,time,capacity,booked_count,status) values ('45000000-0000-0000-0000-000000000001','40000000-0000-0000-0000-000000000001',current_date + 8,'11:00',12,3,'available')
on conflict (id) do update set date=excluded.date,time=excluded.time,capacity=excluded.capacity,booked_count=excluded.booked_count,status=excluded.status,updated_at=now();
SQL
echo "::endgroup::"
...
WINDOW_FROM="$(date -u +%F)"; WINDOW_TO="$(date -u -d '+30 days' +%F)"
assert_rpc_contains "get_public_stay_inventory" "{\"p_stay_id\":\"41000000-0000-0000-0000-000000000001\",\"p_from\":\"${WINDOW_FROM}\",\"p_to\":\"${WINDOW_TO}\"}" '"room_title":"Demo family room"' '"available_count":2' '"price_override":5700'
assert_rpc_contains "get_public_tour_schedules" "{\"p_tour_id\":\"40000000-0000-0000-0000-000000000001\",\"p_from\":\"${WINDOW_FROM}\",\"p_to\":\"${WINDOW_TO}\"}" '"schedule_id":"45000000-0000-0000-0000-000000000001"' '"capacity":12' '"booked_count":3' '"remaining_count":9'

npm run build
npm run start -- -H 127.0.0.1 -p "$APP_PORT" >"$APP_LOG" 2>&1 & APP_PID=$!
ready=0
